...
function analyzeText() {
    ...
    const pronounCounts = {};
    pronouns.forEach(p => pronounCounts[p] = 0);
    let totalPronouns = 0; // To store the count of all pronouns
    tokens.forEach(t => {
        if (pronounCounts.hasOwnProperty(t)) {
            pronounCounts[t]++;
            totalPronouns++; // Increment the total pronouns count
        }
    });
    ...
    const prepositionCounts = {};
    prepositions.forEach(p => prepositionCounts[p] = 0);
    let totalPrepositions = 0; // To store the count of all prepositions
    tokens.forEach(t => {
        if (prepositionCounts.hasOwnProperty(t)) {
            prepositionCounts[t]++;
            totalPrepositions++; // Increment the total prepositions count
        }
    });

    // Indefinite articles
    const articleCounts = { "a": 0, "an": 0 };
    let totalArticles = 0; // To store the count of all indefinite articles
    tokens.forEach(t => {
        if (articleCounts.hasOwnProperty(t)) {
            articleCounts[t]++;
            totalArticles++; // Increment the total articles count
        }
    });
    ...
    output.innerHTML = `
        ...
        <ul>
            ${Object.entries(pronounCounts).map(([k, v]) => `<li>${k}: ${v}</li>`).join("")}
            <li><strong>Total Pronouns:</strong> ${totalPronouns}</li>
        </ul>

        <h2>Prepositions Count</h2>
        <ul>
            ${Object.entries(prepositionCounts).map(([k, v]) => `<li>${k}: ${v}</li>`).join("")}
            <li><strong>Total Prepositions:</strong> ${totalPrepositions}</li>
        </ul>

        <h2>Indefinite Articles Count</h2>
        <ul>
            ${Object.entries(articleCounts).map(([k, v]) => `<li>${k}: ${v}</li>`).join("")}
            <li><strong>Total Indefinite Articles:</strong> ${totalArticles}</li>
        </ul>
    `;
}
